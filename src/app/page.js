...
export default function HomePage() {
  ...
  // 1. First-Time Visitor & Auth Check
  useEffect(() => {
    if (typeof window !== "undefined") {
      const hasVisited = localStorage.getItem("trustanchor_has_visited");
      const loggedInUser = localStorage.getItem("trustanchor_auth_user");
      const params = new URLSearchParams(window.location.search);
      const urlToken = params.get("token");

      // If opening for the first time without an active session or token, route to /login
      if (!hasVisited && !loggedInUser && !urlToken) {
        localStorage.setItem("trustanchor_has_visited", "true");
        router.replace("/login");
        return;
      }

      // Mark visited for future browser opens
      localStorage.setItem("trustanchor_has_visited", "true");
      setCheckingFirstVisit(false);

      // Auto-verify if ?token= parameter is present in URL
      if (urlToken) {
  ...
  if (checkingFirstVisit) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center text-xs text-slate-500 font-mono">
        Initializing TrustAnchor Ledger...
      </div>
    );
  }
